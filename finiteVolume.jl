...
# Flux interpolation
function linInterp_3D(mesh::Mesh, sln::SolutionState)
    nCells, nFaces, nBoundaries, nBdryFaces = unstructuredMeshInfo(mesh)
    ...
    @inbounds @fastmath for f in 1:nFaces-nBdryFaces
        ...
        @views sln.faceFluxes[f, :] .= sln.cellFluxes[c1, :].*(c2Dist/totalDist) .+ sln.cellFluxes[c2, :].*(c1Dist/totalDist)
    end
end
